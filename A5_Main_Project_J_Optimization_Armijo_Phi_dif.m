...
%parameters setting well------------------Please do notchange-----------
n_target=3;
valied_target=3;
Phi_dif=[1 0.8 0.7];
% X_target=[2 2.5 4.5];% Standard
X_target=[2 2.1 4.5];
S=[0 6];
u=[1 -1];
%r=[2 2 1.5];   %2-targets-end
r=[2.3 2 1.5]; %3-targets-end
%r=[2 3 0.5];   %1-target-end
%r=[1.5 1 0.5];    %0-target-end
R=[0 0 0];
Theta=[1 2 3];
W=[1 1.5 0.5]; %3-targets-end

[J,Final_Phi] = J_Computation_dif_Phi(X_target,S,u,r,R,Theta,W,n_target,Phi_dif);
J_list=[];
Theta_list=[];
W_list=[];
Final_Phi_list=[];
...
%(3)Gradient of J
[delta_Theta_J,delta_W_J] = Delta_J_Computation_dif_Phi(X_target,r,R,Theta,W,valied_target,Final_Phi,Phi_dif);
...
[J_Next,Final_Phi] = J_Computation_dif_Phi(X_target,S,u,r,R,Next_Theta,Next_W,n_target,Phi_dif);
if Armijo_Condition == 1
    ...
    while k <= k_bound && J_Next >= J
        ...
        [J_Next,Final_Phi] = J_Computation_dif_Phi(X_target,S,u,r,R,Next_Theta_Ar,Next_W_Ar,n_target,Phi_dif);
...
%(8)Output of Plots
[T_list, R_list, Phi_list, S_list] = Data_collecting_dif_Phi(X_target,S,u,r,R,Theta_best,W_best,n_target,Phi_dif);
J=T_list(end);
color_bar5=[0.6,0.3,0.8]*0.93;
Result_plotout(T_list, R_list, Phi_list, S_list, n_target);
figure(4)
plot([1:iteration_time-200],J_list(200:end),'-','color',color_bar5,'LineWidth',1);
xlabel('Iteration')
ylabel('J Value')
xlim([0,iteration_time-200]);
%ylim([7,10.5]);
%title('J Value Plot')
%fprintf('The final plots are printed.\n');
%fig4 = figure(4);
%saveas(fig4,'C:\Users\12647\Desktop\fig4_J.eps','epsc')
...
%save T_R_Phi_S_Final_List_3target_dif_Phi_2_end.mat T_R_Phi_S_Final_List
